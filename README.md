# 🚀 resumes-site

<p align="center">
  📄 A curated collection of my professional resumes, hosted via GitHub Pages  
</p>

<p align="center">
  <a href="https://abdulmoizshaikh.github.io/resumes-site/">
    <img src="https://img.shields.io/badge/🌐_Live_Site-Visit-blue?style=for-the-badge" />
  </a>
  <img src="https://img.shields.io/badge/Hosted_on-GitHub_Pages-black?style=for-the-badge&logo=github" />
  <img src="https://img.shields.io/badge/Format-PDF-green?style=for-the-badge" />
</p>

---

## 📄 Resume Portfolio

> Each resume is tailored for a specific role and accessible via a dedicated page.

### 🔗 Quick Access

| Role                                | View Resume                                                                                            |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------ |
| 🔹 **Senior React Native Engineer** | [Open](https://abdulmoizshaikh.github.io/resumes-site/Muhammad_Moiz_Senior_React_Native_Engineer.html) |
| 🔹 **Full Stack Engineer**          | [Open](https://abdulmoizshaikh.github.io/resumes-site/Muhammad_Moiz_Full-Stack_Engineer.html)          |
| 🔹 **Senior Frontend Engineer**     | [Open](https://abdulmoizshaikh.github.io/resumes-site/Muhammad_Moiz_Senior_Frontend_Engineer.html)     |
| 🔹 **Backend Engineer**             | [Open](https://abdulmoizshaikh.github.io/resumes-site/Muhammad_Moiz_Backend_Engineer.html)             |

---

## 🌐 Live Preview

👉 https://abdulmoizshaikh.github.io/resumes-site/

---

## 🧠 About This Project

* Built using **GitHub Pages (static hosting)**
* Each resume is an **HTML wrapper around a PDF**
* Designed for **quick sharing with recruiters**
* Fully accessible via direct links

---

## ⚡ Features

* 📱 Mobile-friendly viewing
* ⚡ Fast load (static assets)
* 🔗 Shareable role-specific links
* 📄 Embedded PDF preview

---

## 📌 Usage

You can directly share any resume link depending on the role you're applying for.

Example:

```text
Frontend Role → Use Frontend Resume Link
Backend Role → Use Backend Resume Link
```

---

## ⭐ Future Improvements

* Custom domain (e.g. yourname.dev)
* UI enhancements (cards, thumbnails)
* Analytics tracking (visitors, clicks)

---

<p align="center">
  ⭐ If you find this useful, feel free to star the repo!
</p>
